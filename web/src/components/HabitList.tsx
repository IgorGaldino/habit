...
export function HabitList({ date, onCompletedChanged }: HabitListProps) {
  ...
  async function handleToggleHabit(habitId: string) {
    try {
      ...
      console.error(error);
      alert("Ocorreu um problema no servidor!");
    }
  }

  const isDateIntPast = dayjs(date).endOf("day").isBefore(new Date());
    ...
    <div className="mt-6 flex flex-col gap-3">
      {habitsInfo?.possibleHabits.map((habit) => (
        <Checkbox.Root
          ...
          disabled={isDateIntPast}
          className={clsx("flex items-center gap-3 group focus:outline-none", {
            "cursor-not-allowed": isDateIntPast
          })}
